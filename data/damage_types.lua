...
setDefaultProjector(function(src, x, y, type, dam)
	...
	if target then
		...
		if target == game.player then
			game.logSeen(target, game.flash.BAD, "%s hits me for %s%0.2f %s damage#LAST#.", src.name:capitalize(), DamageType:get(type).text_color or "#aaaaaa#", dam, DamageType:get(type).name)
		 end
		if src == game.player then
			game.logSeen(target, game.flash.GOOD, "I hit %s for %s%0.2f %s damage#LAST#.", target.name, DamageType:get(type).text_color or "#aaaaaa#", dam, DamageType:get(type).name)
		end
